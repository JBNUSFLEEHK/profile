...
--------------------------------------------------------------------
큰 틀
1. “Thingspaek” 사이트에서 실시간으로 데이터를 받는다. 
2. 뽑은 데이터에서 저희 조가 필요로 하는 값만을 뽑아내는 과정을 거친다.
3. 데이터에 공식을 대입해 vpd값을 구한다. 
4. 그래프로 표현한다.
...
------------------------------------------------------------------------------------
(1번과 2번) 
thingspeak 사이트 내에 api key를 공유해주는 위치가 있어 그곳에 들어가 저희 조가 사용하고
있는 라즈베리파일의 api key를 받았습니다. 
그 과정에서 저희는 requests를 사용했고, 특정 url을 입력 후 r = requests.get(url) 과 같은
방식을 사용해 불러왔습니다.
데이터를 불러온 후 필요로 하는 값을 뽑아내는 과정을 거쳐야 했습니다. 
...
def pltGraph라는 함수 내에 x변수에 datelist(날짜), y 변수 내에 vpdlist를 넣어 x가
시간을 다뤄 시간이 지남에 따라 y값에 vpd가 나오도록 함수를 설정했습니다. 
--------------------------------------------------------------------------------
...
--------------------------------------------------------------------------------
(5번)
LineNotify를 통해 적정량 미만 또는 초과일 시, 알림이 오도록 설정하고자 한건데
이 과정은 직접 Line 사이트 내에 들어가 python 구현 방법을 복사하여 사용했습니다. 
하지만 적정량을 설정하는 부분은 저희가 직접 설정을 했는데 if, elif를 통해 적정값에
초과인 상황과 미만인 상황을 설정했고 적정값일 때는 알림이 오지 않도록 했습니다. 
그리고 알림이 매순간 짧은 시간마다 오게 된다면 불편함이 생길 것이기 때문에
time.sleep()부분에서 값을 600seconds, 10분 정도로 설정했습니다.
하지만 이 값은 코드 내에서 자신이 필요하다면 더 짧게, 더 길게 설정하여 바꿀 수 있습니다. 
---------------------------------------------------------------------------------
def main():
