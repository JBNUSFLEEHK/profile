...
------------------------------------------------------------------------------------
(1번과 2번) 
thingspeak 사이트 내에 api key를 공유해주는 위치가 있어 그곳에 들어가 저희 조가 사용하고 있는 라즈베리파일의 api key를 받았습니다. 
그 과정에서 저희는 requests를 사용했고, 특정 url을 입력 후 r = requests.get(url) 과 같은 방식을 사용해 불러왔습니다.
